...
export default function StudyScreen({ openSidebar }) {
  ...
  // DELETE NOTE SYNCED WITH BACKEND
  ...
  // SAVE NOTE SYNCED WITH BACKEND
  const handleSaveNote = async (formValues) => {
    try {
      if (editingNote) {
        const updated = await updateNote(editingNote.id, formValues);
        setNotes((prev) =>
