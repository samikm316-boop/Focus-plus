...
export default function StudyScreen({ openSidebar }) {
  ...
  const groupedNotes = subjects
    ...
    .map((subjectObj) => ({
      ...
      notes: filteredNotes.filter((n) => n.subject === subjectObj.name),
    }))
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "18px" }}>
        <button onClick={openSidebar} style={menuBtn}>
          <Menu size={22} />
        </button>
        <h1 style={titleStyle}>STUDY</h1>
        <div style={{ width: "44px" }} />
      </div>

      {/* TABS */}
      <div style={tabsContainer}>
        <Tab icon="➕" label="Create" active={activeTab === "create"} onClick={() => handleTabChange("create")} />
        <Tab icon="📘" label="Notes" active={activeTab === "notes"} onClick={() => handleTabChange("notes")} />
        <Tab icon="📖" label="Flashcards" active={activeTab === "flashcards"} onClick={() => handleTabChange("flashcards")} />
        <Tab icon="🧪" label="Quiz" active={activeTab === "quiz"} onClick={() => handleTabChange("quiz")} />
        <Tab icon="🎓" label="Learn" active={activeTab === "learn"} onClick={() => handleTabChange("learn")} />
      </div>

      {/* SUBJECTS */}
      <div style={sectionHeader}>
        <h2 style={sectionTitle}>SUBJECTS</h2>
        <span style={{ ...viewAll, cursor: "pointer" }} onClick={() => setSelectedSubject("All")}>
          View all
        </span>
      </div>
        ...
        {subjects
          ...
          .map((subject) => (
            <SubjectCard
              ...
              count={`${notes.filter((n) => n.subject === subject.name).length} notes`}
            />
          ))}
      </div>
      ...
      {activeTab === "create" && (
        <div style={placeholderStyle}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginTop: "20px" }}>
            <button onClick={openCreateModal} style={saveBtn}>Create Note</button>
            <button style={{ ...saveBtn, background: "linear-gradient(135deg,#2563EB,#06B6D4)" }}>Create Flashcard</button>
            <button style={{ ...saveBtn, background: "linear-gradient(135deg,#F97316,#EF4444)" }}>Create Quiz</button>
          </div>
        </div>
      )}
      ...
      {activeTab === "notes" && (
        <>
          {groupedNotes.map((group) => (
            <NoteGroup key={group.subject} subject={group.subject} icon={group.icon} count={`${group.notes.length} notes`}>
              {group.notes.map((note) => (
                <Note key={note.id} note={note} onEdit={() => editNote(note)} onDelete={() => deleteNote(note.id)} />
              ))}
            </NoteGroup>
          ))}
        </>
              ...
              return (
                <div key={subjectObj.name} style={{ marginBottom: "18px" }}>
                  ...
                  {subjectCards.map((card) => (
                    <div key={card.id} style={{ background: "white", padding: "16px", borderRadius: "18px", marginBottom: "10px", boxShadow: "0 10px 25px rgba(0,0,0,0.04)" }}>
                      <div style={{ fontWeight: "800", marginBottom: "8px" }}>{card.question}</div>
                      <div style={{ color: "#6B7280", fontSize: "14px" }}>{card.answer}</div>
                      <div style={{ marginTop: "10px", fontSize: "12px", color: "#7C3AED", fontWeight: "700" }}>{card.type}</div>
                    </div>
                  ))}
                </div>
              );
      ...
      {/* FLOAT BUTTON */}
      <button onClick={openCreateModal} style={floatingBtn}>
        <Plus />
      </button>
      ...
      {showModal && (
        <div style={modalOverlay}>
          <div style={modalBox}>
            ...
            <textarea placeholder="Content" value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} style={{ ...inputStyle, height: "120px", resize: "none" }} />
...
function SubjectCard({ name, count, active, onClick, icon, color }) {
  return (
    <div
      onClick={onClick}
      style={{
        minWidth: "130px",
        background: active ? "#DDD6FE" : color,
        padding: "14px",
        borderRadius: "20px",
        cursor: "pointer",
        transition: "0.25s",
    ...
    >
      ...
      <div style={{ fontSize: "12px", color: "#555", marginTop: "4px" }}>{count}</div>
    </div>
  );
}
...
// Fixed missing parameter wrapper formatting here
function NoteGroup({ subject, count, children, icon }) {
  return (
...
function Note({ note, onEdit, onDelete }) {
  return (
    <div style={{ background: "white", padding: "14px", borderRadius: "16px", marginBottom: "10px", boxShadow: "0 10px 25px rgba(0,0,0,0.04)" }}>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          <Pencil size={16} style={{ cursor: "pointer" }} onClick={onEdit} />
          <Trash2 size={16} color="red" style={{ cursor: "pointer" }} onClick={onDelete} />
        </div>
      </div>
